...
/* http://www.codeguru.com/cpp/controls/treeview/multiview/article.php/c3985/Multi-Column-Tree-View.htm */
...
BOOL CColumnTreeView::Create(LPCTSTR lpszClassName, LPCTSTR lpszWindowName, DWORD dwStyle, const RECT& rect, CWnd* pParentWnd, UINT nID, CCreateContext* pContext)
{
	ASSERT ( m_Tree.m_hWnd == NULL );
	ASSERT ( m_Header.m_hWnd == NULL );
	...
	DWORD dwTreeStyle = dwStyle & 0xFFFF;

	BOOL bRet = CView::Create(lpszClassName, lpszWindowName, dwStyle, rect, pParentWnd, nID, pContext);
	if ( !bRet )
		return bRet;

	m_Tree.Create(WS_CHILD | WS_VISIBLE | TVS_NOHSCROLL | TVS_NOTOOLTIPS | dwTreeStyle, CRect(), this, TreeID);
